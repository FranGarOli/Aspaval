...
class EventRequest extends FormRequest
{
    ...
    {
        return [
            // 'name' => ['required', 'min:5', 'max:255', 'unique:events'],
            'name' => ['required', 'min:5', 'max:15'],
            'description' => ['required', 'min:5', 'max:5000'],
            'location' => ['required', 'min:5', 'max:255'],
            'date' => ['required', 'after:today'],
            'hour' => ['required'],
            'tags' => ['required', 'min:3', 'max:25'],
        ];
    }
    ...
    public function messages()
    {
        return [
            ...
            'date.after' => 'La fecha debe ser actual o posterior a hoy',
            'hour.required' => 'La hora es obligatoria',
            'tags.required' => 'El tag es obligatorio.',
